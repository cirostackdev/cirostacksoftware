// Script to replace all 17 remaining template entries in industries-generated.ts
const fs = require('fs');
const filepath = 'C:/Users/USER/Desktop/cirostackagency-next/cirostackwebsite/src/data/industries-generated.ts';
let content = fs.readFileSync(filepath, 'utf8');

// Helper: normalize line endings for matching, then use replace with regex
function replaceEntry(industry, oldFragment, newFragment) {
    // Try direct replacement first
    if (content.includes(oldFragment)) {
    ...
    if (normalized.includes(oldFragment)) {
        const replaced = normalized.replace(oldFragment, newFragment);
        content = replaced.replace(/\n/g, '\r\n');
        console.log(`✓ ${industry} (normalized)`);
        return true;
    }
...
// ============ TUTORING SERVICES ============
replaceEntry('tutoring-services challenges+rest',
`        challenges: [
